...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
    ...
    for iteration in range(first_iter, final_iter + 1):
        ...
        gt_image_tensor = torch.cat(gt_images, 0).to(image_tensor.device)
        # Loss
        # breakpoint()
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations) or iteration % saveAtiter == 0:
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint,
    ...
    timer = Timer()
    scene = Scene(dataset, gaussians, load_coarse=None) # Gaussian initialization
    timer.start()
    # coarse stage
...
def pil_show(img, save_dir=None):
    from PIL import Image
    img = img.permute(1, 2, 0).cpu().numpy()
    img = (img * 255).astype(np.uint8)
    Image.fromarray(img).show()
    if save_dir:
        Image.fromarray(img).save(save_dir)
        print("save image to", save_dir)
...
if __name__ == "__main__":
    ...
    # Display the concrete index of GPU.
    if torch.cuda.is_available():
        print(f'GPU: {torch.cuda.get_device_name()}, and now we have {torch.cuda.device_count()} GPUs!')
    ...
    parser.add_argument("--configs", type=str, default="arguments/dnerf/bouncingballs.py")

    # add by yunfei
    # lp._model_path = "output/1080_Dance_Dunhuang_Single_f14"
    # lp._source_path = "/mnt/d/common/Datasets/PKU/part1/1080_Dance_Dunhuang_Single_f14/data_COLMAP/000000"
    # lp._source_path = '/mnt/d/common/Datasets/PKU/part1/1080_Dance_Dunhuang_Single_f14/per_frame/000000'
    # lp.gpu = 0
    args = parser.parse_args(sys.argv[1:])
    args.save_iterations.append(args.iterations)

    # save expname as a new folder
    if os.path.exists(args.expname):
        index = 1
        while os.path.exists(args.expname + f'-exp{index}'):
            index += 1
        add_postfix = f'-exp{index}'
        args.expname = args.expname + add_postfix
    if args.configs:
        # import mmcv
